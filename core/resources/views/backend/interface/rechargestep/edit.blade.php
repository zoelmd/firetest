        ...
        <div class="col-md-12">
            <div class="portlet light bordered">
                ...
                <div class="portlet-body">
                    <form method="POST" action="{{route('admin.recharegeStep.update',$step->id)}}" class="clearfix" ENCTYPE="multipart/form-data">
                        {{ method_field('PUT') }}
                        {{ csrf_field() }}
                        <input type="hidden" name="previous_order_id" id="previous_order_id" value="{{$step->order}}">
                         <input type="hidden" name="previous_step_id" id="previous_step_id" value="">
                        <div class="form-group {{ $errors->has('title') ? ' has-error' : '' }}">
                            ...
                            <div class="input-group">
                                ...
                                <input type="text" id="title" class="form-control has-error " name="title" placeholder="Feature Title" value="{{$step->name }}" required>
                            </div>
                            @if ($errors->has('title'))
                         ...
                         <div class="form-group {{ $errors->has('order') ? ' has-error' : '' }}">
                            ...
                            <div class="input-group">
                                ...
                                <select name="order_id" id="order_id" class="form-control">
                                    @foreach( $steps as $s)
                                            @if($s->id==$step->id)
                                                <option value="{{$s->order}}" data-value="{{$s->id}}" selected="" class="form-control">{{$s->order}} - {{$s->name}}</option>
                                            @else
                                                <option value="{{$s->order}}" data-value="{{$s->id}}" class="form-control">{{$s->order}} - {{$s->name}}</option>          
                                            @endif
                                    @endforeach
                                </select>
                            </div>
                        ...
                        <div class="form-group {{ $errors->has('icon') ? ' has-error' : '' }}">
                            ...
                            <div class="input-group">
                                ...
                                <input type="text" id="icon" class="form-control has-error " name="icon" placeholder="Font Awesome icon code with fa fa-, eg: 'address-card'" value="{{$step->icon }}" required>
                            </div>
                            @if ($errors->has('icon'))
                                <span class="help-block">
                                    {{ $errors->first('icon') }}
                        ...
                        <div class="form-group {{ $errors->has('description') ? ' has-error' : '' }}">
                            ...
                            <textarea id="description" class="form-control" name="description" rows="5">{!!$step->content!!}</textarea>
                        ...
                        <div class="form-group">
                            ...
                            @if($step->status==0)
                            <input id="status" name="status" type="checkbox" class="make-switch"  data-on-color="success" data-off-color="danger" checked="">
                            @else
    ...
    <script>
        $(document).ready(function() {
            $(document).on('change','#order_id',function(){
                $('#previous_step_id').val($('#order_id option:selected').attr('data-value'));
            });
